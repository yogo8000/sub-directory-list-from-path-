...
def generate_subdirectories_list(path, directories_list):
    """
    This function updates a list received as input with all subdirectories
    full paths.
    ...
    loop on all the subdirectories in the current directory(It gets the list using
    the os.listdir method) and it calls The function recursively with the path, file separator
    and the subdirectory name all concatenated.
    when the function gets to a "dead end" meaning no subdirectory exists, It appends it to
    the subdirectories list.
    :param path: The current path on which we want to append its subdirectories.
    :param directories_list: The list of the subdirectories path.
    ...
    if os.path.isdir(path):
        ...
        for dir in os.listdir(path):
            generate_subdirectories_list(path + OS_FILE_SEP + dir, directories_list)
...
def directory_to_list(directory_path):
    ...
    :return: The function returns a list of all paths of the subdirectories of the path given
    as input.
    """
    ...
    generate_subdirectories_list(path=directory_path, directories_list=directories_list)
